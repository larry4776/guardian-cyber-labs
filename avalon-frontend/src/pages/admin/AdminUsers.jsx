    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
        ...
        <AdminTopBar
          ...
          subtitle={<>7 livrables en attente · <span style={{ color: '#c0505a' }}>3 dépassent 48h</span></>}
        />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
            <div style={{ display: 'flex', gap: '8px' }}>
              {FILTRES.map(f => (
                <button key={f} onClick={() => setActiveFilter(f)} style={{
                  ...
                  fontSize: '13px', fontWeight: activeFilter === f ? '600' : '400', cursor: 'pointer',
                }}>{f}</button>
              ))}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <select style={{ appearance: 'none', padding: '8px 32px 8px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', fontSize: '13px', cursor: 'pointer', outline: 'none' }}>
          ...
          <div style={{ ...card, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtered.map((l, i) => (
                  <tr key={i} style={{ borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', background: l.enRetard ? 'rgba(192,80,90,0.05)' : 'transparent' }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ fontSize: '11px', fontWeight: '700', padding: '4px 10px', borderRadius: '6px', color: l.statut === 'En cours' ? '#4a7fc2' : '#c9a94e', background: l.statut === 'En cours' ? 'rgba(74,127,194,0.15)' : 'rgba(201,169,78,0.15)' }}>{l.statut}</span>
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      <button onClick={() => navigate(`/admin/correction/${l.id}`)} style={{ fontSize: '12px', fontWeight: '600', padding: '7px 14px', borderRadius: '6px', border: '1px solid rgba(59,109,240,0.4)', background: '#3b6df0', color: '#fff', cursor: 'pointer' }}>Corriger</button>
                    </td>
                  </tr>
                ))}
              </tbody>
